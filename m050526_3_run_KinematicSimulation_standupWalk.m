...
T = 1; % swing time
standupTime = 4; % stand up time 2s
...
dp_cog_vel = [0.0205/(4*npoints);0.00000;0.00000];
...
for cycle=1:cycles % how many cycles of gait
    ...
    for legs=[2,1,4,3] % leg sequence
        ...
        for i=1:npoints-1 % going through trajectory points
            ...
            JJ = [JC;J;Jcog];
            W = diag([1000*ones(k, 1);1000*ones(3,1); [100; 100; 0.1]]);

            % only 
            % JJ = [JC;J;Jcog(1:2,:)];
            % W = diag([10*ones(k, 1);1000*ones(3,1); [0.1; 0.1]]);

            % with CoG manipulation
            ...
            % compute all the q
            qall = ( (W*JJ) \ (W * [zeros(k,1); dp; dp_cog]) );
            qr(:, count+1) = qr(:, count) + qall(1:8);
            B(:, count+1) = B(:, count) + qall(9:14);
