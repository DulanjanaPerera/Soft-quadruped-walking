function y = gait_v2(u)
%#codegen
% instead of relying on sample time, we use the previous time from the
% input and use it. The trajectories are generated for \previous time step
% and current time step (t-dt, t)
...
if isTouched
    ...
    W = diag([1000*ones(k,1); 100*ones(3,1); 1*ones(3,1)]); % 15x15
    ...
    leg_idx = mod(floor( double(time)/double(T) ), numel(legseq)) + 1;
    legs    = legseq(leg_idx);
...
y = [FL; BL; FR; BR; Bdy; qr_len; Bsen; time];
