...
T = 1; % swing time
standupTime = 2; % stand up time 2s
holdTime = 1; % hold time 1s
dt = 0.05; % descretization
npoints = round(T/dt);% number of walking points
Nstand = round(standupTime/dt); % number of point for standing up
Nhold = round(holdTime/dt); % number of points for hold
tvec = 0:dt:T;
time_vec = 0.0;
...
B = zeros(6,npoints*4*cycles  + Nstand + Nhold); % body frame trajectory
...
qr = zeros(8, npoints*4*cycles + Nstand + Nhold);
qr(:,1) = [l(2); l(3); l2(2); l2(3); l(2); l(3); l(2); l(3)];
wf_x = zeros(3, npoints*4*cycles + Nstand + Nhold);
...
swingLegHist = zeros(1, size(qr,2));
...
for i = 1:Nstand
    ...
    qr(:,count) = [l2;l3;l2;l3;l2;l3;l2;l3];
    B(:,count)  = [0;0;p(1);0;0;0];
    ...
    swingLegHist(count) = 0;
    count = count + 1;
    time_vec(count) = time_vec(count-1) + dt;
end
...
for cycle=1:cycles % how many cycles of gait
    ...
    for legs=[2,1,4,3] % leg sequence
        ...
        for i=1:npoints-1 % going through trajectory points
            ...
            % Current actual CoG
            Tcog = global_COG(qr(:,count), 0, B(:,count), b, 1, L, r);
            x_cog_actual = Tcog(1:2,4);
            
            % Stance feet
            stanceIdx = setdiff(1:4, legs);
            stanceXY = footPos(1:2, stanceIdx);
            
            % Nominal figure-8 displacement
            ...
            dp_nom_xy = (COG_next(1:2) - COG_now(1:2)) + dp_cog_vel(1:2);
            
            % Nominal next CoG
            x_nom_next = x_cog_actual + dp_nom_xy;
            
            % Project nominal target into safe support triangle
            margin = 0.005;   % 5 mm, tune this
            x_safe_next = projectPointToShrunkenTriangle(x_nom_next, stanceXY, margin);
            
            % Final CoG command
            dp_cog = x_safe_next - x_cog_actual;
            ...
            % JJ = [JC;J;Jcog];
            JJ = [JC;J;Jcog(1:2,:)];
            % W = diag([10*ones(k, 1);1000*ones(3,1); [0.1; 0.1; 0.1]]);
...
figure(2)
plot(time_vec(1:end_time),B(1,1:end_time), time_vec(1:end_time),B(2,1:end_time), time_vec(1:end_time),B(3,1:end_time));
grid on
axis tight
xlabel 'time (s)'
ylabel 'Cartesian'
...
figure(3)
plot(time_vec(1:end_time),wf_x(1,1:end_time), time_vec(1:end_time),wf_x(2,1:end_time), time_vec(1:end_time),wf_x(3,1:end_time));
grid on
axis tight
xlabel 'time (s)'
ylabel 'Cartesian'
...
% animateQuadrupedFastToVideo(qr, B, b, L, r, rBody, dt, "standing_walking_v2");

% plot the CoG and foot trajecotry.
data = plotCoGFootXYTrajectories(qr(:, 1:end_idx), B(:, 1:end_idx), b, L, r, 'SwingLeg', swingLegHist(1:end_idx), 'PlotSupport', false);
