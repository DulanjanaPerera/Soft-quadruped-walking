% This is damped base null space version.
...
dp_cog_vel = [stepLen_l/(4*npoints);0.00000;0.00000];
...
for cycle=1:cycles % how many cycles of gait
    ...
    for legs=[2,1,4,3] % leg sequence
        ...
        for i=1:npoints % going through trajectory points
            % if i == 100
            %     disp(i);
            % end
            if legs==1
            ...
            % JJ = [JC;J;Jcog];
            JJ = [JC;J;Jcog(1:2,:)];
            % W = diag([100*ones(k, 1);1000*ones(3,1); [0.1; 0.1; 0.1]]);
            ...
            % % damped update
            rhs = [zeros(k,1); dp; dp_cog(1:2)];
            delta14 = localDampedLS(W*JJ, W*rhs, 1e-8);
            dq = delta14(1:8);
            qr(:, count+1) = qr(:, count) + dq;

            % Base update from stance constraints. Damped LS is safer than raw
            % backslash when JC(:,9:end) is close to rank-deficient.
            A_base = JC(:,9:14);
            b_base = JC(:,1:8) * dq;
            dB = localDampedLS(A_base, b_base, 1e-6);
            B(:, count+1) = B(:, count) - dB;
...
% animateQuadrupedFastToVideo(qr(:, 1:end_idx), B(:, 1:end_idx), b, L, r, rBody, dt, "standing_walking_v4");
