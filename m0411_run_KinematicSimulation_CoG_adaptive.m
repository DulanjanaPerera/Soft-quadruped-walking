...
dp_cog = [0.04/(4*npoints);0.000;0.00000];
...
for cycle=1:cycles % how many cycles of gait
    ...
    for legs=[2,1,4,3] % leg sequence
        ...
        for i=1:npoints-1 % going through trajectory points
            ...
            if legs==1
                if i == 1
                    % robot frame foot position
                    Tfoot = global_leg1HTM(qr(1:2, count), 1.0, zeros(6,1), b, 0.0, L, r);
            ...
            elseif legs ==2
                if i == 1
                    % robot frame foot position
                    Tfoot = global_leg2HTM(qr(3:4, count), 1.0, zeros(6,1), b, 1.0, L, r);
                    % initialize the trajectory
                    traj = swingTrajectory_initializer_simulink(1, stepLen_l, Tfoot(1:2,4), tvec(i));
                    P_d_l = swingTrajectory_evaluator_simulink(r, L, T, [tvec(i),tvec(i+1)], traj);
                else
            ...
            elseif legs==3
                if i == 1
                    % robot frame foot position
                    Tfoot = global_leg3HTM(qr(5:6, count), 1.0, zeros(6,1), b, 0.0, L, r);
                    % initialize the trajectory
                    traj = swingTrajectory_initializer_simulink(-1, stepLen_r, Tfoot(1:2,4), tvec(i));
                    P_d_r = swingTrajectory_evaluator_simulink(r, L, T, [tvec(i),tvec(i+1)], traj);
                else
                    P_d_r = swingTrajectory_evaluator_simulink(r, L, T, [tvec(i),tvec(i+1)], traj);
                end
                flipy = eye(4);
            ...
            elseif legs==4
                if i == 1
                    % robot frame foot position
                    Tfoot = global_leg4HTM(qr(7:8, count), 1.0, zeros(6,1), b, 1.0, L, r);
                    % initialize the trajectory
                    traj = swingTrajectory_initializer_simulink(-1, stepLen_r, Tfoot(1:2,4), tvec(i));
                    P_d_r = swingTrajectory_evaluator_simulink(r, L, T, [tvec(i),tvec(i+1)], traj);
                else
